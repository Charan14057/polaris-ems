"""
POLARIS-EMS — Wind Generation Forecaster
SIH26061: Polar Energy Management & Resilience System

Forecasts wind generation across operational horizons (1-48h).
Enforces physical turbine curve constraints:
- Zero generation below cut-in speed or above storm cut-out speed
- Clamped above by station-specific turbine rated capacity from StationProfileRegistry
...
class WindForecaster:
    """XGBoost wind forecaster with hard turbine curve and cut-out constraints."""
    ...
    def __init__(
        ...
        limits = self.station_config.get_wind_limits(self.station_id)
        self.wind_rated_kw = float(limits["wind_turbine_kw_rated"])
        self.cut_in_ms = float(limits["wind_cut_in_speed_ms"])
        self.cut_out_ms = float(limits["wind_cut_out_speed_ms"])
    ...
    def fit(
        ...
        """Fits wind point and quantile models."""
    ...
    def _apply_physical_bounds(
        ...
        """Enforces turbine cut-in, cut-out, and rated capacity clamps."""
        pred = np.clip(raw_pred, 0.0, self.wind_rated_kw)
        
        # Check wind speed column
        wind_col = "forecast_wind_speed_ms" if "forecast_wind_speed_ms" in X.columns else "origin_wind_speed_ms"
        if wind_col in X.columns:
            w_vals = X[wind_col].values
            # Cut-in zeroing: below cut-in or above cut-out
            cut_mask = (w_vals < self.cut_in_ms) | (w_vals > self.cut_out_ms)
            pred[cut_mask] = 0.0
